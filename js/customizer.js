...
(() => {
	...
	wp.customize("header_textcolor", (value) => {
		value.bind((to) => {
			document
				.querySelectorAll(".site-title, .site-description")
				.forEach((el) => {
					if ("blank" === to) {
						 ...
						 */
						const isValidHex =
							/^#(?:[0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/.test(to);

						if (!isValidHex && typeof console !== "undefined") {
							console.warn(
								"Canard customizer.js: header_textcolor value rejected (not a hex color):",
								typeof to,
							);
						}
