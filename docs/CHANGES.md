...
| **posts.js** | **Bug fix — Jetpack infinite scroll event name corrected:** The `'post-load'` event listener has been replaced with `'inf_scr_posts_loaded'`. `'post-load'` is a legacy Jetpack event deprecated and no longer fired as of Jetpack 9.2; the current event is `'inf_scr_posts_loaded'`. As a result, `format-image` and `format-gallery` posts injected by infinite scroll beyond the first page never had their background images applied. |
| **posts.js** | **Bug fix — thumbnail height deferred via `requestAnimationFrame`:** `postThumbnail.style.height` was calculated from `entry.offsetHeight` synchronously at the moment the `'inf_scr_posts_loaded'` event fired. Newly injected posts have not yet been laid out by the browser at that point, so `offsetHeight` returned `0` and the thumbnail height was set to a negative number, collapsing the image. The height read is now deferred in a `requestAnimationFrame` loop that retries until a non-zero layout height is available (typically one frame). |
